...
describe("测试 details 页面", () => {
    ...
    //测试 saveUser 函数
    test("测试 saveUser 方法", () => {
        const mockFns = jest.fn();
        const mockFns1 = jest.fn();
        const mockFns2 = jest.fn();
        mockFns();
        mockFns1();
        mockFns2();
        wrapper.setMethods({goBack:mockFns,storeData:mockFns1,userDetail:mockFns2});//模拟方法
        ...
        expect(mockFns).toHaveBeenCalled();//确保方法执行
        expect(mockFns1).toHaveBeenCalled();//确保方法执行
        expect(mockFns2).toHaveBeenCalled();//确保方法执行

        expect(mockFn).toHaveBeenCalled(); //查看方法是否回调
        expect(mockFn1).toHaveBeenCalled(); //查看方法是否回调
    });
    ...
    test("测试 goBack 方法", () => {
        const mockFns = jest.fn();
        mockFns();
        wrapper.setMethods({goBack:mockFns});//模拟方法
